...
When somebody calls \inlinecode{addEvent()} we create a \inlinecode{PerThreadData} object on demand and try to push the event into the queue. If the queue is full we wait a moment so that the worker (writing) thread can free the queue.
...
Unfortunately the original patch and patchclean tools had some implementation specific limitations which made a direct port to UNIX hardly feasible.  They did use the DIA API and \inlinecode{CALLMONTOOLS::PatchWriter} uses Windows specific code to write the actual executable file.
...
Because of the above mentioned reasons we decided to re-implement the patching tool chain for Linux and Mac OS X.  This 2 new UNIX patching tools reside in the \pathname{factextraction/unix/\\unixfactextraction/} directory in the UNIXFACTEXTRACTION namespace.  Figure \ref{fig:unixfe_unixfe} gives an overview of all the involved classes.

The logic for patching and cleaning happens in the 2 files \pathname{main\_patch.cpp} and \pathname{main\_patchclean.cpp}.  Both files are very similar and use the loosely coupled to above classes.  In the following paragraphs each of the involved classes gets a more detailed description to illustrate the implementation of the patching process on UNIX platforms.

\subsubsection{Class Overview}

\paragraph{Function and Callsite}

This 2 classes are structs with convenience methods and constructors, they are the basic objects which the UNIX fact extraction library handles.  A \inlinecode{Function} bundles a number of \inlinecode{Callsite}s, we are doing this because one function might contain several enter and exit instrumentations and the actual number of instrumentations varies.
...
The to retriever classes gather information about executable files including shared objects on Linux and dylibs on Mac OS X.  The \inlinecode{BaseAddressRetriever} in particular is responsible for getting the base address and the offset in the actual file of an executable file's text segement and provides the static method \inlinecode{getCallsites()} for this task.

This is information is required for the \inlinecode{ExeFilePatcher} to modify the files at the right offsets.  Since this task is highly platform specific because Linux uses the ELF binary format and Mac OS X uses Mach-O there is no generic way to achieve this, there are also no generic POSIX APIs which could help here.
...
On Linux we use \toolname{objdump -d} and on Mac OS X \toolname{otool -tvV} to disassemble the text segments of the binaries.  Both tools generate different but quite similar outputs.  So we did not have to implement 2 separate parsing mechanisms but rather use one parser functions which can handle both output formats.

\paragraph{ExeFilePatcher}

The \inlinecode{ExeFilePatcher} is the 
